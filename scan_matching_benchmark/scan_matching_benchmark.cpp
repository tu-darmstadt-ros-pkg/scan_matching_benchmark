...
ScanMatchingBenchmark::ScanMatchingBenchmark(ros::NodeHandle &nh)
{
  ...
  TestSetGenerator generator(0.0121312402188778);
  //generator.generateCuboid(pointcloud, 1.4, 1.4, 2.0);
  //generator.generateCylinder(pointcloud, 0.7, 1.8);

  //generator.generateHalfCylinderHalfCube(pointcloud, 1.1942321062, 1.1942321062, 2.0182981491);
  generator.loadPCDDir(pointcloud, "a");
  std::cout<<"Finished Generation"<<std::endl;

  const cartographer::transform::Rigid3d initial_pose_estimate = cartographer::transform::Rigid3d::Translation({0,0,0});//{-0.479261905,	0.8737565875,	0.0828114077});
  ...
  cartographer::sensor::PointCloud projected_cloud = cartographer::sensor::TransformPointCloud(pointcloud, initial_pose_estimate.cast<float>());
...
BatchScanMatchingBenchmark::BatchScanMatchingBenchmark(ros::NodeHandle &nh)
{
  ...
  float max_initial_error = 1.5001;
