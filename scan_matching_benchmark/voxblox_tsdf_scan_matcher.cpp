...
VoxbloxTSDFScanMatcher::VoxbloxTSDFScanMatcher(ros::NodeHandle &nh, ScanMatcherConfig config)
{
  ...
  if(config.publish_cloud) {
    ...
    gradient_x_publisher_ = nh.advertise<sensor_msgs::PointCloud2>("voxblox_tsdf_gradient_x", 1, true);
    gradient_y_publisher_ = nh.advertise<sensor_msgs::PointCloud2>("voxblox_tsdf_gradient_y", 1, true);
    ros::spinOnce();
  }
...
void VoxbloxTSDFScanMatcher::evaluateScanMatcher(const cartographer::sensor::PointCloud& scan_cloud,
  ...
  cartographer::mapping_3d::scan_matching::CeresVoxbloxTSDFScanMatcher voxblox_scan_matcher(ceres_scan_matcher_options, config_.cubic_interpolation, config_.boundary_extrapolation);
    ...
    for(float x = min_x; x <= max_x; x += 0.01) {
      for(float y = min_y; y <= max_y; y += 0.01) {
        for(float z = min_z; z <= max_z; z += 0.01) {
          ...
          std::vector<cartographer::mapping_3d::scan_matching::PointCloudAndVoxbloxTSDFPointers> point_and_grid = {{&scan_point, voxblox_tsdf}};
